...
const errorOptions = ref(['Pantalla en blanco', 'Emision Parada', 'Winbox sin internet', 'Streaming pausado', 'otro ...']);
const herramientaOptions = ref(['.Net', 'Escritorio remoto', 'Soporte remoto', 'Soporte presencial' , 'otro ...']);
...
const saveData = () => {
  clientData.value[selectedCliente.value] = [...rows.value];
  console.log('Data saved for', selectedCliente.value, ':', clientData.value[selectedCliente.value]);
};

const changeCliente = (event) => {
  selectedCliente.value = event.target.value;
  rows.value = [...clientData.value[selectedCliente.value]];
  console.log('Cliente cambiado a:', selectedCliente.value, 'Datos cargados:', rows.value);
};

const viewData = () => {
  savedRows.value = [...clientData.value[selectedCliente.value]];
};
</script>
...
<template>
  <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-2">
    ...
    <div class="flex justify-end mt-4 space-x-2">
      ...
      <button @click="saveData" class="px-4 py-2 bg-blue-500 text-white rounded-md">Guardar Datos</button>
      <button @click="viewData" class="px-4 py-2 bg-yellow-500 text-white rounded-md">Ver Datos</button>
    </div>
    <div v-if="savedRows.length > 0" class="mt-8">
      <h3 class="text-lg font-medium text-gray-900 dark:text-white">Datos Guardados para {{ selectedCliente }}:</h3>
      <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 mt-4">
        ...
        <tbody>
          <tr v-for="(row, index) in savedRows" :key="index" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
            <td class="px-6 py-4">{{ row.winbox }}</td>
            <td class="px-6 py-4">{{ row.falla }}</td>
            <td class="px-6 py-4">{{ row.tipoError }}</td>
            <td class="px-6 py-4">{{ row.herramienta }}</td>
            <td class="px-6 py-4">{{ row.fechaReporte }}</td>
            <td class="px-6 py-4">{{ row.fechaSolucion }}</td>
            <td class="px-6 py-4">{{ row.tiempoResolucion }}</td>
          </tr>
        </tbody>
      </table>
    </div>
